...
import { createStagedMediaImport, scanPastMediaImport } from "@/features/archdiocese/past-media/service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (files.length > 40) {
      return NextResponse.json({ error: "Import up to 40 images at a time." }, { status: 400 });
    }
    ...
    for (const file of files) {
      try {
        const staged = await createStagedMediaImport({ context, file });
        results.push({ id: staged.id, filename: file.name, status: "uploaded" });

        try {
          const scanned = await scanPastMediaImport({ importId: staged.id, context, hierarchy });
          results[results.length - 1].status = scanned.review_status;
        } catch (scanError) {
          results[results.length - 1].status = "failed";
          results[results.length - 1].error = scanError instanceof Error ? scanError.message : "Media scan failed.";
        }
      } catch (uploadError) {
        results.push({
          filename: file.name,
