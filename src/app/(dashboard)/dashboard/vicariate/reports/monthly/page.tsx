...
export default async function VicariateMonthlyReportsPage() {
  ...
  const reports = await getMonthlyReports(context.archdioceseId, "vicariate", context.vicariateId);
  const now = new Date();
  ...
  return (
    <VicariateShell
      ...
      actions={
        <GenerateReportButton
          ...
          scopeEntityId={context.vicariateId}
          year={currentYear}
          month={currentMonth}
    ...
    >
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <StatCard label="Reports" value={reports.length} helper="Vicariate monthly reports" icon={BarChart3} />
      ...
      {reports.length ? (
        <SimpleTable title="Report archive" rows={reports} columns={[
          { header: "Period", cell: (r) => <Button href={`/dashboard/vicariate/reports/monthly/${r.id}`} variant="ghost" className="h-auto px-0 py-0 font-medium">{MONTH_LABELS[r.reportMonth - 1]} {r.reportYear}</Button> },
        ...
        <EmptyState title="No monthly reports yet" description="Generate the first monthly report for this vicariate." action={
          <GenerateReportButton
            scopeLevel="vicariate"
            scopeEntityId={context.vicariateId}
            year={currentYear}
