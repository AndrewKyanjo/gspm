...
export default async function VicariateMonthlyReportDetailPage({ params }: Props) {
  ...
  return (
    <VicariateShell
      ...
      subtitle={`${report.scopeName ?? "Vicariate"} • ${report.status}`}
      actions={
        <div className="flex flex-wrap gap-2 print:hidden">
          <PrintButton />
          <Button href="/dashboard/vicariate/reports/monthly" variant="secondary">All reports</Button>
      ...
      role={context.role}
    >
      <PageHeader title={`${MONTH_LABELS[report.reportMonth - 1]} ${report.reportYear} — Monthly Report`} description={`Composite report for ${report.scopeName ?? "the vicariate"}.`} actions={<Badge>{report.status}</Badge>} />
      ...
      <section className="space-y-4">
        ...
        {financialData.byParish.length > 0 && (
          <SimpleTable title="Per parish breakdown" rows={financialData.byParish} columns={[
