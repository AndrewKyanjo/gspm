...
import { uploadDeaneryMedia, type DeaneryMediaUploadState } from "@/features/deanery/media/actions";

const initialState: DeaneryMediaUploadState = { error: null };
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob(
        (value) => {
          ...
          else reject(new Error("Could not compress the selected image."));
        },
        "image/webp",
        0.58
      );
...
export function DeaneryMediaUploadForm() {
  const [state, action, pending] = useActionState(uploadDeaneryMedia, initialState);
